  ...
  return (
    <div className="min-h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          {
            label: 'Clientes registrados', valor: '—',
            icono: (
          ...
          {
            label: 'Pedidos pendientes', valor: '—',
            icono: (
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl p-6" style={glass}>
          ...
          <div className="space-y-3">
            {[
              { texto: 'Sistema iniciado correctamente', tiempo: 'Ahora' },
              { texto: 'Panel listo para operar', tiempo: 'Hace un momento' },
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: '#1D9E75' }}></div>
                <div>
                  <p className="text-sm text-[#1F2A24]/70">{item.texto}</p>
                  <p className="text-xs text-[#1F2A24]/35 mt-0.5">{item.tiempo}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Estado del sistema */}
        <div className="rounded-2xl p-6" style={glass}>
          <h2 className="text-[#1F2A24] font-medium mb-4">Estado del sistema</h2>
          <div className="space-y-3">
            {[
              { label: 'Base de datos', estado: 'Conectada', ok: true },
              { label: 'Servidor', estado: 'Operativo', ok: true },
              { label: 'Correo', estado: 'Activo', ok: true },
            ].map((item, i) => (
              <div key={i} className="flex items-center justify-between">
                <span className="text-sm text-[#1F2A24]/60">{item.label}</span>
                <span
                  className="text-xs px-2.5 py-1 rounded-full flex items-center gap-1.5"
                  style={{
                    background: item.ok ? 'rgba(29,158,117,0.10)' : 'rgba(220,38,38,0.10)',
                    border: `1px solid ${item.ok ? 'rgba(29,158,117,0.2)' : 'rgba(220,38,38,0.2)'}`,
                    color: item.ok ? '#1D9E75' : '#dc2626',
                  }}
                >
                  <span className="w-1.5 h-1.5 rounded-full" style={{ background: item.ok ? '#1D9E75' : '#dc2626' }}></span>
                  {item.estado}
                </span>
              </div>
            ))}
