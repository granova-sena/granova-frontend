...
function ConfigurarPedidoPage() {
  ...
  // Resultado post-confirmación según método de pago:
  // 'pasarela' → se redirige a /pagar (pantalla aparte).
  // 'verificacion' → transferencia/efectivo (pago pendiente de revisión).
          ...
          {pasoActual === 2 && (
            <div className="w-full min-w-0 flex-1 rounded-xl p-5 sm:p-8 border border-white/15 bg-white/[0.08] backdrop-blur-xl flex flex-col items-center justify-center gap-4 py-16">
              {!idPedido ? (
                ...
                <>
                  <div className="w-16 h-16 bg-[#D8A92E]/15 rounded-full flex items-center justify-center text-3xl">
                    ⏳
                  </div>
              ...
              ) : pagoResultado?.tipo === 'contra_entrega' ? (
                <>
                  <div className="w-16 h-16 bg-[#1D9E75]/15 rounded-full flex items-center justify-center text-3xl">
                    🚚
                  </div>
                  ...
                  <p className="text-xs text-white/40 text-center max-w-xs">
                    💵 Se cobra al momento de la entrega. No necesitas pagar ahora.
                  </p>
                  <button
                    type="button"
                    onClick={() => navigate('/cliente/pedidos')}
