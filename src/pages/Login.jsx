...
const handleLogin = async (e) => {
  ...
  if (!formData.email.trim() || !formData.contraseña.trim()) {
    toast.error('todos los campos son obligatorios', { id: 'error-login' })
    return
  }

  const emailValido = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)
  if (!emailValido) {
    toast.error('Ingresa un correo electrónico válido', { id: 'error-login' })
    return
  }
    ...
    try {
      ...
      if (!respuesta.ok) {
        toast.error(datos.error || 'Error al iniciar sesion')
        return
      }
...
>
  ← Volver
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          <img src={logo} alt="Granova logo" className="w-9 h-9 object-contain object-top" />
          <span className="text-[#E1F5EE] text-xl font-medium tracking-tight">Granova</span>
        </div>

        <h2 className="text-2xl font-medium text-white mb-1 text-center">Bienvenido de nuevo</h2>
        <p className="text-sm text-white/60 mb-8 text-center">Inicia sesión en tu cuenta de Granova</p>

        <form onSubmit={handleLogin}>

       <button type="button" onClick={iniciarLoginGoogle} className="w-full flex items-center justify-center gap-3 py-2.5 rounded-xl text-sm text-white hover:bg-white/10 transition mb-6" style={{background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)'}}>
  ...
  Continuar con Google
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-white/40">o continúa con tu correo</span>
            <div className="flex-1 h-px bg-white/15"></div>
          </div>
    ...
    {verContraseña ? (
      ...
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        ...
        <circle cx="12" cy="12" r="3" stroke="currentColor" strokeWidth="2"/>
          ...
          <p className="text-right text-xs text-[#9DC9B4] mb-6 cursor-pointer hover:underline" onClick={() => navigate('/olvide-password')}>
  ¿Olvidaste tu contraseña?
          ...
          <p className="text-center text-sm text-white/50">
            ¿No tienes cuenta?{' '}
            <span className="text-[#9DC9B4] cursor-pointer hover:underline" onClick={() => navigate('/register')}>
              Regístrate aquí
            </span>
          </p>
