...
function SimuladorInterno() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      try {
        ...
        if (!cancelado) {
          const frescos = eliminarDuplicados(json.data.map(adaptarProducto));
          setProductos(frescos);
          setDescuentosVolumen(json.descuentosVolumen || []);
          // Cuando llega el catálogo fresco, refresca los datos (precios/stock)
          // de las líneas restauradas desde localStorage sin tocar la cantidad
          // ni el formato elegido.
          setLineas(prev => prev.map(l => {
            const fresco = frescos.find(p => String(p.id) === String(l.p.id));
            return fresco ? { ...l, p: fresco } : l;
          }));
        }
      } catch (err) {
  ...
  // Persistir la simulación cada vez que cambia (memoria entre visitas).
  useEffect(() => {
    try {
      localStorage.setItem(claveSimulacion(), JSON.stringify(lineas));
  ...
  function enviarLineasAlCarrito() {
    construirItemsCotizacion().forEach(agregarAlCarrito);
  }

  function llevarAlCarrito() {
    enviarLineasAlCarrito();
    // Los productos ya viajaron al carrito: se vacía la simulación para que no
    // queden duplicados si el cliente vuelve a "Llevar al carrito".
    setLineas([]);
    try { localStorage.removeItem(claveSimulacion()); } catch { /* noop */ }
  ...
  return (
    <div className="min-h-screen text-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        {/* Encabezado — la cotización vive aquí (parte superior) */}
        <FadeIn>
