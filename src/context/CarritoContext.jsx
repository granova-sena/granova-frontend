...
export function CarritoProvider({ children }) {
  ...
  try {
    ...
    const pedidoCreado = await crearPedido(body)

    if (metodoPago === 'nequi') {
      setIdPedidoActual(pedidoCreado.id_pedido)
    }
    ...
    return {
      ...
      estado: pedidoCreado.estado ?? null,
      estado_pago: pedidoCreado.estado_pago ?? null,
      total: pedidoCreado.total ?? null,
      mensaje: pedidoCreado.mensaje ?? null,
      descuento_aplicado: pedidoCreado.descuento_aplicado ?? 0,
      descuento_fuente: pedidoCreado.descuento_fuente ?? null,
      puntos_ganados: pedidoCreado.puntos_ganados ?? 0,
    }
  } catch (error) {
  const mensajeBackend = error.response?.data?.mensaje
  console.error('Error confirmando pedido:', mensajeBackend || error.message)
  return { ok: false, mensaje: mensajeBackend || error.message }
}
}
  ...
  const sincronizarCarrito = (productosExternos) => {
    const productosAdaptados = productosExternos.map(p => ({
      id: p.id,
      nombre: p.nombre,
      presentacion: p.presentacion || p.origen || '',
      precio: p.precio,
      ...
      promo_pct: p.promo_pct ?? p.promoPct ?? null,
