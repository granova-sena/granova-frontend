...
// Mensajes flotantes de confirmación (toast) para el resultado del pago
...
export function useEstadoPago(idTransaccion) {
  ...
  useEffect(() => {
    ...
    async function consultar() {
      try {
        ...
        if (esFinal) {
          ...
          // Toast flotante de confirmación — solo una vez por transacción
          ...
          return; // deja de programar el siguiente intento
        }
    ...
    return () => {
      cancelado = true; // evita actualizar estado si el componente se desmontó
    };
  }, [idTransaccion]);
